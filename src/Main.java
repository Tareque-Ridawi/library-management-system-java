...
public class Main {
                ...
                } else if (password.equals("")) //If password is null
                {
                    ...
                    try {
                        ...
                        if (!rs.next()) { // Check if there is at least one row
                            ...
                            f.dispose();
                            // Process the first row (already fetched by rs.next())
                            do {
                            ...
                            } while (rs.next()); // Process remaining rows
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
    ...
    public static Connection connect() {
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/library?user=root&password=admin");
            ...
            while (resultSet.next()) {
                ...
                if (databaseName.equals("library")) {
                    ...
                    Statement stmt = connection.createStatement(
                            ...
                            ResultSet.CONCUR_READ_ONLY // Read-only result set
                    );
